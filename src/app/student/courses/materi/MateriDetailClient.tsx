...
import {
  ...
  File, ExternalLink, ZoomIn, RefreshCw, Table2
} from 'lucide-react';
...
function CSVPreview({ url, fileName, fileSize }: { url: string; fileName?: string; fileSize?: number | null }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(99,102,241,0.2)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: 'rgba(30,30,46,0.98)', borderBottom: '1px solid rgba(99,102,241,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: '0.82rem', color: '#94a3b8', fontWeight: 600 }}>{fileName || 'data.csv'}</span>
          {fileSize && <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: 10, background: 'rgba(99,102,241,0.1)', color: '#a5b4fc', border: '1px solid rgba(99,102,241,0.2)' }}>{(fileSize / 1024).toFixed(1)} KB</span>}
        ...
        <button onClick={() => downloadFile(url, fileName || 'data.csv')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 8, padding: '6px 12px', color: '#a5b4fc', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '3rem', background: 'rgba(15,15,25,0.8)', flexDirection: 'column' }}>
          ...
          <button onClick={() => downloadFile(url, fileName || 'data.csv')} style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 8, padding: '8px 14px', color: '#a5b4fc', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>
...
function FilePreviewSection({
  ...
  const isCSV = ext === 'csv' || (fileName && fileName.toLowerCase().endsWith('.csv'));
  ...
  // ── CSV file ──────────────────────────────────────────────────────────────────
  if (isCSV) {
    return <CSVPreview url={url} fileName={fileName} fileSize={fileSize} />;
  }
